...
class BudgetServiceTest {
    ...
    void shouldRejectDuplicatedBudgetOnUpdate() {
        ...
        BudgetDTO movedDto = new BudgetDTO(
                budgetId, categoryId, CATEGORY_NAME, CATEGORY_ICON, YEAR, MONTH + 1, new BigDecimal("120.00"), false);
    ...
    void shouldUpdateBudgetSuccessfully() {
        ...
        BudgetDTO updateDto = new BudgetDTO(
                budgetId, categoryId, CATEGORY_NAME, CATEGORY_ICON, YEAR, MONTH, new BigDecimal("250.00"), false);
    ...
    @Test
    @DisplayName("Should carry a recurring budget over to later months that have no budget of their own")
    void shouldCarryRecurringBudgetOver() {
        Budget recurring = recurringBudgetOf(new BigDecimal("300.00"), YEAR, 1);

        when(securityUtils.getCurrentUser()).thenReturn(mockUser);
        when(transactionRepository.getExpensesByCategoryWithIcon(eq(mockUser), any(), any()))
                .thenReturn(List.<Object[]>of(new Object[]{CATEGORY_NAME, CATEGORY_ICON, new BigDecimal("30.00")}));
        when(budgetRepository.findByUserAndYearAndMonth(mockUser, YEAR, MONTH)).thenReturn(List.of());
        when(budgetRepository.findRecurringStartingOnOrBefore(mockUser, YEAR, MONTH)).thenReturn(List.of(recurring));

        BudgetProgressDTO progress = singleProgress();

        assertEquals(recurring.getId(), progress.budgetId(), () -> "The recurring row itself must be reported");
        assertTrue(progress.recurring(), () -> "Reading must be flagged as recurring");
        assertEquals(1, progress.month(), () -> "Reading must expose the budget's own start month");
        assertEquals(0, new BigDecimal("300.00").compareTo(progress.limitAmount()), () -> "Limit mismatch");
        assertEquals(10.0, progress.percentage(), 0.0001, () -> "Spending is measured against the carried-over limit");
    }

    @Test
    @DisplayName("Should let a budget created for the exact period override a recurring one")
    void shouldPreferExactPeriodOverRecurring() {
        Budget exact = budgetOf(new BigDecimal("100.00"));
        Budget recurring = recurringBudgetOf(new BigDecimal("300.00"), YEAR, 1);

        when(securityUtils.getCurrentUser()).thenReturn(mockUser);
        when(transactionRepository.getExpensesByCategoryWithIcon(eq(mockUser), any(), any())).thenReturn(List.of());
        when(budgetRepository.findByUserAndYearAndMonth(mockUser, YEAR, MONTH)).thenReturn(List.of(exact));
        when(budgetRepository.findRecurringStartingOnOrBefore(mockUser, YEAR, MONTH)).thenReturn(List.of(recurring));

        BudgetProgressDTO progress = singleProgress();

        assertEquals(exact.getId(), progress.budgetId(), () -> "Exact period must win over the recurring budget");
        assertFalse(progress.recurring());
        assertEquals(0, new BigDecimal("100.00").compareTo(progress.limitAmount()), () -> "Limit mismatch");
    }

    @Test
    @DisplayName("Should pick the newest recurring budget when several start before the period")
    void shouldPreferNewestRecurringBudget() {
        Budget older = recurringBudgetOf(new BigDecimal("200.00"), YEAR - 1, 6);
        Budget newer = recurringBudgetOf(new BigDecimal("400.00"), YEAR, 3);

        when(securityUtils.getCurrentUser()).thenReturn(mockUser);
        when(transactionRepository.getExpensesByCategoryWithIcon(eq(mockUser), any(), any())).thenReturn(List.of());
        when(budgetRepository.findByUserAndYearAndMonth(mockUser, YEAR, MONTH)).thenReturn(List.of());
        // The repository query orders newest start first; the service relies on that order.
        when(budgetRepository.findRecurringStartingOnOrBefore(mockUser, YEAR, MONTH)).thenReturn(List.of(newer, older));

        BudgetProgressDTO progress = singleProgress();

        assertEquals(newer.getId(), progress.budgetId(), () -> "Newest start must take over the category");
        assertEquals(0, new BigDecimal("400.00").compareTo(progress.limitAmount()), () -> "Limit mismatch");
    }

    /** A recurring budget for the fixture category starting at the given period. */
    private Budget recurringBudgetOf(BigDecimal limitAmount, int year, int month) {
