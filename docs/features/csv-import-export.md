...
- Row-level validation: valid date, positive amount, `INCOME`/`EXPENSE`/`TRANSFER` type, account must exist for the user (matched by exact name — no match means the row is invalid). `category` is mandatory for `INCOME`/`EXPENSE` and optional for `TRANSFER`, mirroring `TransactionDTO`'s own validation. A category name that does not exist for the user is **not** an error: the preview flags the row (`ImportRowDTO.newCategory`) and lists the distinct names in `ImportPreviewDTO.newCategories`; the commit creates each one once (exact name match, `CategoryIconCatalog.DEFAULT_ICON`) before persisting the rows that use it. `destination_account` only applies to `TRANSFER` rows: mandatory, must exist for the user, and must differ from `account` — for `INCOME`/`EXPENSE` rows the column is ignored even if present. Duplicate detection via `TransactionRepository.existsByUserAndDateAndAmountAndDescriptionAndType`.
